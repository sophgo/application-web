...
func DeleteHistoryLog() {
	...
	dir, _ := filepath.Abs(filepath.Dir("/var/log/application-web/"))
	// 获取目录下所有文件
	files, err := filepath.Glob(filepath.Join(dir, "application-web.log.*"))
	if err != nil {
	...
	for _, file := range files {
		...
		dateStr := fileName[len("application-web.log."):]
